...
// @ts-expect-error - Ignoring type error for Netlify deployment
...
export const t = (key: string): string => {
		...
		const normalizedPrayerName =
			prayerName === 'first-third'
				...
				: prayerName === 'last-third'
					...
					: prayerName;
		return (
			prayerNames[lang as keyof typeof prayerNames]?.[
				normalizedPrayerName as keyof (typeof prayerNames)['en']
			] || prayerName
		);
	}
	...
	const normalizedKey =
		key === 'first-third' ? 'first_third' : key === 'last-third' ? 'last_third' : key;

	// For regular keys, first check our additionalTranslations
	const additional =
		additionalTranslations[lang as keyof typeof additionalTranslations]?.[
			normalizedKey as keyof (typeof additionalTranslations)['en']
		];
	if (additional) {
		return additional;
	}
	...
	if (key === 'until') {
		return lang === 'ru' ? 'до' : lang === 'id' ? 'sampai' : lang === 'zh' ? '直到' : 'until';
	}
	...
	if (key === 'error_fetching') {
		return lang === 'ru'
			? 'Не удалось получить время молитв'
			: lang === 'id'
	...
	try {
		// @ts-expect-error - Cast needed for Netlify deployment
		const messageFunction =
			paraglideMessages && typeof paraglideMessages === 'object' ? paraglideMessages[key] : null;
