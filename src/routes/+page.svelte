<script lang="ts">
	...
	let motionStatus = 'checking'; // 'checking', 'requesting', 'granted', 'denied', 'not-needed'
	...
	function updateColorsBasedOnCurrentPrayer() {
		...
		if (browser) {
			const accentColorValue =
				prayerColors[currentPrayer as keyof typeof prayerColors] || prayerColors.dhuhr;
			const gradientValue =
				prayerGradients[currentPrayer as keyof typeof prayerGradients] || prayerGradients.dhuhr;
			...
			if (gradientColors.length > 0) {
				document.documentElement.style.setProperty(
					'--gradient-color-1',
					gradientColors[0] || accentColorValue
				);
				document.documentElement.style.setProperty(
					'--gradient-color-2',
					gradientColors[1] || accentColorValue
				);
				document.documentElement.style.setProperty(
					'--gradient-color-3',
					gradientColors[2] || accentColorValue
				);
			}
	...
	async function setupMotionDetection() {
		...
		try {
			...
			if (
				...
				motionStatus = 'denied'; // Show "tap to enable" for iOS
			} else {
	...
	// Manual trigger for iOS permission (fallback)
	function triggerMotionPermission() {
		if (
			isIOS &&
			typeof DeviceMotionEvent !== 'undefined' &&
			'requestPermission' in DeviceMotionEvent
...
<div
	class="header {isHeaderVisible ? 'visible' : ''}"
	on:mouseenter={showHeader}
	on:mouseleave={hideHeader}
	role="banner"
>
...
<div
	class="carousel {isHeaderVisible ? 'header-visible' : ''} {isSwiping
		? 'swiping-active'
		: ''} {isCarouselMode ? 'carousel-mode' : ''}"
...
>
	{#if isMobile}
		<img
			src="/favicon.png"
			alt="akh logo"
			class="mobile-logo {isSwiping ? 'swiping' : ''} {isCarouselMode ? 'carousel' : ''}"
		/>
	{/if}

	<!-- Carousel mode indicator (top left, only in carousel mode) -->
	<div class="carousel-mode-indicator {isCarouselMode ? 'visible' : ''}">Carousel Mode</div>
	...
	<div class="page-switcher {isCarouselMode ? 'visible' : ''}">
		<div class="page-items">
			{#each pageNames as name, i}
				<div
					class="page-item {i === currentPageIndex ? 'active' : ''}"
	...
	{#if isMobile && motionStatus === 'denied'}
		<div class="motion-status {motionStatus}" on:click={triggerMotionPermission}>
			❌ Tap to Enable
		</div>
	{/if}
</div>
